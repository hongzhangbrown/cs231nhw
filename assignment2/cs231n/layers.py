...
def affine_forward(x, w, b):
    ...
    xnew = np.reshape(x,(x.shape[0],dim))
    out = np.dot(xnew,w)+b
    cache = (x, w, b)
    return out, cache
...
def affine_backward(dout, cache):
    ...
    dim = np.prod(x.shape[1:])
    xnew = np.reshape(x,(x.shape[0],dim))
    temp = np.dot(dout,np.transpose(w))
    dx, dw, db =np.reshape(temp,x.shape) , np.dot(np.transpose(xnew),dout), np.sum(dout,axis = 0)
    return dx, dw, db
...
def conv_forward_naive(x, w, b, conv_param):
 ...
 # if not H_prime.is_integer() or not W_prime.is_integer():
  ...
  out = np.dot(W_col,X_col) + b.reshape((len(b),1))
  out = out.reshape(F, H_prime, W_prime, N)
...
def conv_backward_naive(dout, cache):
  ...
  dX_col = np.dot(np.transpose(W_reshape), dout_reshaped)
